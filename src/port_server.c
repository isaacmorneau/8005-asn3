...
void epoll_server(const char * port, bool m) {
    ...
    int efd;
    directional_buffer * dirbuf_in, * dirbuf_out;
        ...
        for (i = 0; i < n; i++) {
            if (events[i].events & EPOLLERR) {
                ...
                while (1) {
                    ...
                    ensure(dirbuf_in = calloc(1, sizeof(directional_buffer)));
                    ensure(dirbuf_out = calloc(1, sizeof(directional_buffer)));
                    //TODO you need to make connected based on the populated pairs
                    init_directional_buffer(dirbuf_in, dirbuf_out, infd, outfd);

                    //round robin client addition

                    event.data.ptr = dirbuf_in;
                    ensure(epoll_ctl(epollfds[epoll_pos % total_threads], EPOLL_CTL_ADD, infd, &event) != -1);
                    event.data.ptr = dirbuf_out;
